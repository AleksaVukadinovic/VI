...
def Min(game, alpha = float('inf'), beta = float('-inf')):
    ...
    for next_state in get_next_states(game):
        opp_best_move_value, _ = Max(next_state, alpha, beta)

        if opp_best_move_value < best_move_value:
            best_move_value = opp_best_move_value
...
def Max(game, alpha = float('inf'), beta = float('inf')):
    ...
    for next_state in get_next_states(game):
        opp_best_move_value, _ = Min(next_state)
...
def end(current_state: XOState):
    return current_state.move_count == 9 or get_winner(current_state) is not None
